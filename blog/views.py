...
def detail(request, pk):
	...
	post = get_object_or_404(Post, id=pk)  # get_object_or_404 方法，其作用就是当传入的 pk 对应的 Post 在数据库存在时，
...
# 首先是需要根据从 URL 中捕获的分类 id（也就是 pk）获取分类，这和 category 视图函数中的过程是一样的。不过注意一点的是，在类视图中，
# 从 URL 捕获的命名组参数值保存在实例的 kwargs 属性（是一个字典）里，非命名组参数值保存在实例的 args 属性（是一个列表）里。所以我们
# 使了 self.kwargs.get('pk') 来获取从 URL 捕获的分类 id 值。然后我们调用父类的 get_queryset 方法获得全部文章列表
#     def get_queryset(self):
